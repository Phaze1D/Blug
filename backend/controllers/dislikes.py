...
class DislikesController(object):
    ...
    def create(self, post_id):
        ...
        if dislike.is_valid() && not Dislike.already_exist(dislike.post.id(), dislike.user.id()):
    ...
    def delete(self, dislike_id):
        ...
        dislike = Dislike.get_by_id(long(dislike_id))
        if dislike and dislike.user.id() == current_user_id()
            dislike.key.delete()
